...
test("restored storefront and admin product workflow", async () => {
  ...
  const sample = database.prepare("SELECT p.id, p.price, p.main_image, p.source_id, c.slug FROM products p JOIN categories c ON c.id = p.category_id WHERE p.status = 1 ORDER BY p.id DESC LIMIT 1").get();
